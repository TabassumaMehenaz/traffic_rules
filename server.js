...
app.use(cors());
// Serve static files from the 'frontend' folder, assuming server.js is in 'backend'
app.use(express.static(path.join(__dirname, '../frontend')));

// Models
const Complaint = require('./models/complaint');
const User = require('./models/user');
const QuizQuestion = require('./models/quizQuestion'); // ADDED: Import the new QuizQuestion model
...
// NEW: Route to serve quiz questions from MongoDB
app.get('/api/quiz-questions', async (req, res) => {
  try {
    const questions = await QuizQuestion.find({}); // Fetch all questions from the database
    // Optional: Shuffle questions before sending to frontend
    const shuffledQuestions = questions.sort(() => 0.5 - Math.random());
    res.json(shuffledQuestions);
  } catch (error) {
    console.error("Error fetching quiz questions from DB:", error);
    res.status(500).json({ error: "Failed to fetch quiz questions." });
  }
});

// TEMPORARY: Route to populate initial quiz questions (RUN ONCE)
// REMOVE THIS ROUTE AFTER YOU HAVE POPULATED YOUR DATABASE
